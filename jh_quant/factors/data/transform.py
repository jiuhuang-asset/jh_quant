...
def daily_to_monthly(
    ...
    monthly_df = df.groupby(['symbol', 'year_month'], sort=False).agg(agg_rules).reset_index()
    ...
    if trade_dates is not None and 'trade_date' in trade_dates.columns:
        ...
        monthly_df = monthly_df.merge(month_end_trade_dates, on='year_month', how='left')
...
def calculate_cumulative_returns(
    ...
    df['cumulative_return'] = (1 + df[return_column]).groupby(df['symbol']).cumprod() - 1
...
def get_month_end_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    month_end = df.groupby(['symbol', 'year_month'], sort=False)['date'].max().reset_index()
