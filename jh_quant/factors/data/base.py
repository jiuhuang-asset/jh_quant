...
class FactorReturnData(ABC):
    ...
    def __init__(
        ...
        self.api_url = api_url or getenv(
    ...
    def _init_mock_data(self):
        ...
        self._mock_dates = pd.date_range(
            start=self.mock_start, end=self.mock_end, freq="B"
        ).tolist()
    ...
    def _prepare_market_cap(
        ...
        if period == TimePeriod.MONTHLY:
            ...
            market_cap = (
                monthly.groupby(["symbol", "year_month"], sort=False)
                .agg(
                    {
                        "total_mv": "last",
                        "date": "last",
    ...
    def _calculate_roe_from_financials(
        ...
        lrb = self.get_financial_data(
            "LRB", start_date, end_date, symbols, trade_date=trade_date
        )
        zcfz = self.get_financial_data(
            "ZCFZ", start_date, end_date, symbols, trade_date=trade_date
        )
        ...
        lrb_agg = (
            lrb.groupby(["symbol", "date"], sort=False)
            .agg({"ni": "last"})
            .reset_index()
        )
        zcfz_agg = (
            zcfz.groupby(["symbol", "date"], sort=False)
            .agg({"be": "last"})
            .reset_index()
        )
    ...
    def _get_industry(self, symbols: Optional[List[str]] = None) -> pd.DataFrame:
        """
        获取行业信息数据。
    ...
    def get_financial_data(
        ...
        for col in df.columns:
            if col not in cols_to_keep and df[col].dtype in [
                "float64",
                "int64",
                "float32",
                "int32",
...
class FF5Data(FactorReturnData):
    ...
    def prepare_data(
        ...
        **kwargs,
        ...
        asset_growth_data = self._calculate_asset_growth_from_financials(
...
class HOU_XUE_ZHANGData(FactorReturnData):
    ...
    def prepare_data(
        ...
        **kwargs,
        ...
        # 季度ROE数据
        roe_quarterly_data = self._calculate_roe_quarterly(
            start_date, end_date, symbols
        )
        # 资产增长率
...
class CAPMData(FactorReturnData):
    ...
    def _calculate_market_excess_return(
        self, index_data: pd.DataFrame, shibor_data: pd.DataFrame, period: TimePeriod
    ) -> pd.DataFrame:
        """
        计算市场超额收益率
        ...
        if period == TimePeriod.MONTHLY:
            ...
            shibor_by_month = (
                shibor_monthly.groupby("year_month")
                .agg(
                    {
            ...
            result = index_monthly.merge(
                shibor_by_month[["date", "rf_monthly"]], on="date", how="left"
            )
            ...
            result = index_daily.merge(
                shibor_daily[["date", "rf_daily"]], on="date", how="left"
            )
