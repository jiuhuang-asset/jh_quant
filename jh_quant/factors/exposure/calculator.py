...
class StockExposureCalculator:
    ...
    def _calculate_rolling_for_symbol(
        ...
        merged = pd.DataFrame({
            'date': stock_dates,
            'return': stock_data['return'].values
        }).merge(
            pd.DataFrame({'date': factor_dates}).assign(**{col: factor_matrix[col].values for col in factor_cols}),
            on='date',
            how='inner'
        ).sort_values('date')
    ...
    def calculate_rolling_exposures(
        ...
        if verbose:
            print(f'Calculating rolling betas for {len(symbols)} stocks, lookback={lookback_months} periods...')

        stock_groups = {
            symbol: grp.sort_values('date').reset_index(drop=True)
            for symbol, grp in stock_returns[stock_returns['symbol'].isin(symbols)].groupby('symbol', sort=False)
        }
        ...
        if self.n_jobs == 1:
            ...
            with ThreadPoolExecutor(max_workers=self.n_jobs) as executor:
                results = list(executor.map(lambda args: self._calculate_rolling_for_symbol(*args), tasks))
    ...
    def calculate_all_exposures(
        ...
        if self.n_jobs == 1:
            ...
            with ThreadPoolExecutor(max_workers=self.n_jobs) as executor:
                results = list(executor.map(lambda args: self._calculate_for_symbol(*args), tasks))
    ...
    def _calculate_for_symbol(
        ...
        merged = pd.DataFrame({
            ...
            pd.DataFrame({'date': factor_dates}).assign(**{col: factor_returns[col].values for col in factor_cols}),
            on='date',
            how='inner'
        ).sort_values('date')
