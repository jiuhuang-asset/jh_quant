...
## 19. Multi-Service Management (v2)
...
```json
{
  "config_bundle": { "service": {}, "selection_spec": null, "strategy_specs": [], "portfolio_spec": {} },
...
Service status (same as `GET /service/status`).

### `POST /services/{session_id}/scheduler/start`

Start scheduler (same as `POST /service/scheduler/start`).

### `POST /services/{session_id}/scheduler/stop`

Stop scheduler (same as `POST /service/scheduler/stop`).

### `POST /services/{session_id}/run-once`

Execute one cycle (same as `POST /service/run-once`).

### `GET /services/{session_id}/performance`

Performance report (same as `GET /service/performance`).

### `GET /services/{session_id}/runtime`

Runtime snapshot (same as `GET /service/runtime`).

### `GET /services/{session_id}/config`

Config snapshot (same as `GET /service/config`).
...
Response fields:
- `generated_at`, `count`, `note` (when auto-limited)
- `sessions: PerformanceComparisonItem[]`

`PerformanceComparisonItem`:
- Snapshot: `session_id`, `mode`, `initial_capital`, `strategy_names`, `portfolio_value`, `total_return_pct`
