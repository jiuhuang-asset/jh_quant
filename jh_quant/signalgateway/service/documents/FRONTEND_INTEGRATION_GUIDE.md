...
当使用 `MultiServiceManager` 同时运行多个不同策略配置的服务时，前端需要支持对比不同 session 的绩效。
...
1. `GET /health`
2. `GET /services` — 服务列表
3. `GET /services/compare` — 实时对比快照
4. `GET /services/performance/compare` — 历史绩效对比（含净值曲线）
...
`GET /services/performance/compare` 返回的每个 session 都包含 `equity_curve` 数组，
可直接用于 ECharts 多条折线 overlay：

```ts
const { sessions } = await fetchPerformanceComparison()
...
从 `ComparisonSummary` 或 `PerformanceComparisonItem` 的快照字段中提取：
- `total_return_pct`, `max_drawdown`, `win_rate`, `total_pnl`
- 以表格形式展示，按 `total_return_pct` 降序排列

### 16.6 session_ids 选择控件

当服务数量超过 limit 时，API 默认只返回最新的 N 个。建议前端提供：
- 一个多选下拉框，选项来自 `GET /services` 返回的 `services[].session_id`
- 选中后传入 `?session_ids=A,B` 精确查询
